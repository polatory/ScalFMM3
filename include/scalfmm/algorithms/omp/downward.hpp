...
#pragma omp task untied default(none) firstprivate(group_child, parent_groups)                                         \
  shared(approximation, level_interpolator_index)                                                                      \
    depend(iterator(it = 0                                                                                             \
                    : std::size(group_child->csymbolics().group_dependencies_l2l_in)),                                 \
           in                                                                                                          \
           : (group_child->csymbolics().group_dependencies_l2l_in.at(it))[0]) depend(inout                             \
                                                                                     : group_child_raw[0])             \
      priority(prio)
