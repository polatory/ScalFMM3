...
namespace count_kernels
{
    namespace particles
    {
        ...
        struct count_approximation
        {
            ...
            using matrix_kernel_type = count_matrix_kernel;
            using storage_type = scalfmm::component::grid_storage<double, Dimension, 1, 1>;
        ...
        inline void p2p_inner(count_matrix_kernel const& /*mat*/, LEAF_T& target_leaf)
        {
        ...
        template<typename Leaf, typename ContainerOfLeafIterator, typename ArrayT, typename ValueT>
        inline void p2p_outer(count_matrix_kernel const&, Leaf& target_leaf, ContainerOfLeafIterator const& neighbors,
                              ArrayT const&, ValueT const&)
        {
            using value_type = typename Leaf::value_type;
            value_type nb_val{0};
        ...
        template<typename Leaf, typename ContainerOfLeafIterator, typename ArrayT, typename ValueT>
        inline void p2p_outer(count_matrix_kernel const&, Leaf& target_leaf, ContainerOfLeafIterator const& neighbors,
                              const int& number_of_neighbors, ArrayT const&, ValueT const&)
        {
            using value_type = typename Leaf::value_type;
            value_type nb_val{0};
            std::for_each(std::begin(neighbors),
                          std::begin(neighbors) + number_of_neighbors
                          /*target_leaf.csymbolics().number_of_neighbors*/,
                          [&nb_val](auto const& leaf)
                          {
                              {
                                  nb_val += static_cast<value_type>(leaf->size());
...
namespace scalfmm::interpolation
{
    ...
    struct interpolator_traits<count_kernels::particles::count_approximation<Dimension>>
    {
        ...
        //         using buffer_value_type = double;
        //         // the type of the element inside the buffer
        //         using buffer_inner_type = xt::xarray<buffer_value_type>;
        //         // matrix of size 2x(number of symmetries) (2 to store  the multipole and the local)
        //         using buffer_shape_type = xt::xshape<2, interpolation::number_of_matrices_in_orthant<dimension>()>;
