...
namespace scalfmm::operators
{
                                ...
                                [[maybe_unused]] ArrayT const& pbc, ValueT const& box_width)
    {
        ...
        // ranges
	//        using position_range = typename particle_type::range_position_type;
	//        using inputs_range = typename particle_type::range_inputs_type;
	//        using outputs_range = typename particle_type::range_outputs_type;
        // value type
        ...
        using simd_tuple_outputs_type = decltype(meta::to_tuple(meta::replace_inner_tuple_type_t<xsimd::simd_type, outputs_type>{}));
	...
	static constexpr auto alignment_tag = simd::unaligned{};
          //std::conditional_t<std::is_same_v<leaf_type, component::leaf_view<particle_type>>, simd::unaligned,
          //                   simd::aligned>{};
        //
        // compute the center of the target_leaf
        const auto target_center = target_leaf.center();
	    ...
	    //            const auto mutual_coefficient = matrix_kernel.template mutual_coefficient<simd_type>();
            ...
            for(std::size_t target_idx = 0; target_idx < target_size; ++target_idx)
            {
                ...
                // simd computation
                {
                    ...
                    {
                        // load simd position
                        auto neighbor_position = simd::load_position<simd_position_type>(neighbor_position_iterator, alignment_tag);
                        ...
                        // load mutual coef in simd
                        auto mutual_coefficient = matrix_kernel.template mutual_coefficient<simd_type>();
#ifdef SCALFMM_BUILD_PBC
    ...
    // =====
    ///
    /// \brief p2p_inner compute the particle-to-particle interactions into the target leaf.
    ///
    ///
    ...
    ///
    /// \param[in] matrix_kernel the kernel used to compute the interaction
    /// \param[inout] target_leaf the current leaf where we compute the particles-to-particles interactions
    ///
    template<typename MatrixKernel, typename Leaf>
    inline void p2p_inner(MatrixKernel const& matrix_kernel, Leaf& target_leaf)
    {
        using leaf_type = Leaf;
        using particle_type = typename leaf_type::particle_type;
        using position_type = typename leaf_type::position_type;
                ...
                for(; neighbor_idx < vec_size; neighbor_idx += inc)
                {
                    ...
                    for(std::size_t m = 0; m < km; ++m)
                    {
                        for(std::size_t n = 0; n < kn; ++n)
                        {
                            ...
                            // to_update_target_outputs.at(n) = xsimd::fma(kxy_nm, neighbor_inputs_values.at(m), tmp);
                            target_outputs.at(n) += kxy_nm * neighbor_inputs_values.at(m);
                            to_update_neighbor_outputs.at(n) +=
                              mutual_coefficient.at(n) * kxy_nm * target_inputs_values.at(m);
                        }
    ...
    inline void p2p_outer(MatrixKernel const& matrix_kernel, TargetLeaf& target_leaf,
                          SourceLeaf const& source_leaf, [[maybe_unused]] ArrayT const& shift)
    {
#ifndef NDEBUG
