...
namespace scalfmm::component
{
    ...
    class group_tree_view
    {
      ...
      public:
        ...
        /// @param number_of_elements_per_group : blocking on the leaves/cells
        group_tree_view(std::size_t tree_height, std::size_t order, std::size_t number_of_elements_per_group,
                        std::size_t number_of_cells_per_group, box_type const& box)
          : m_tree_height(tree_height)
          , m_top_level((box.is_periodic() ? 0 : 2))
          , m_order(order)
          , m_number_of_leaves_per_group(number_of_elements_per_group)
